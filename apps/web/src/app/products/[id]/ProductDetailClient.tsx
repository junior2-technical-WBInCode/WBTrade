...
export default function ProductDetailClient({
  ...
  const [canReviewInfo, setCanReviewInfo] = useState<CanReviewResult | null>(
    null,
  );
  ...
  const handleAttributeChange = (key: string, value: string) => {
    ...
    const exact = variants.find((v) =>
      variantAttributes.every((k) => next[k] && v.attributes?.[k] === next[k]),
    );
    if (exact) {
  ...
  useEffect(() => {
    ...
    async function fetchSameWarehouse() {
      try {
        const response = await productsApi.getSameWarehouseProducts(
          product.id,
          { limit: 20 },
        );
  ...
  useEffect(() => {
    async function fetchReviews() {
      ...
      try {
        const [reviewsResponse, statsResponse, canReviewResponse] =
          await Promise.all([
  ...
  const handleMarkHelpful = async (reviewId: string, helpful: boolean) => {
    ...
    try {
      ...
      const newVotes = {
        ...votedReviews,
        [reviewId]: helpful ? ("helpful" as const) : ("not_helpful" as const),
      };
      setVotedReviews(newVotes);
      try {
        localStorage.setItem("review_votes", JSON.stringify(newVotes));
      } catch {}
    } catch (error) {
  ...
  const isB2b =
    user &&
    ((user as any).b2bStatus === "APPROVED" ||
      (user as any).b2bStatus === "SUSPENDED");
  const b2bMultiplier = isB2b ? (user as any).b2bPriceMultiplier || 1.1 : null;
  ...
  const hasDiscount =
    ...
    Number(product.compareAtPrice) > Number(effectivePrice);
  const discountPercent = hasDiscount
    ? Math.round(
        (1 - Number(effectivePrice) / Number(product.compareAtPrice)) * 100,
      )
    : 0;
  ...
  const isOutletProduct = (() => {
    ...
    return (
      sku.startsWith("OUTLET-") ||
      blId.startsWith("outlet-") ||
      tags.some((t: string) => t.toLowerCase() === "rzeszów")
    );
  })();

  // Use real category if available
  const categoryName = product.category?.name
    ? cleanCategoryName(product.category.name)
    : "Produkty";
  const categorySlug = product.category?.slug || "";
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 mb-6 sm:mb-8">
          ...
          <div className="md:col-span-1 lg:col-span-2 min-w-0">
            <div className="bg-white dark:bg-secondary-800 rounded-xl sm:rounded-2xl p-2 sm:p-4 relative overflow-hidden">
              ...
              {/* Main Image */}
              {/* Dodałem klasę 'group' do głównego diva, aby móc pokazywać strzałki po najechaniu myszką */}
              <div className="aspect-square overflow-hidden rounded-xl sm:rounded-2xl mb-3 sm:mb-4 max-w-full relative bg-white group">
                ...
                {/* Pokazujemy strzałki tylko wtedy, gdy jest więcej niż 1 zdjęcie */}
                {proxiedImages && proxiedImages.length > 1 && (
                  <>
                    {/* Przycisk W Lewo (Poprzednie) */}
                    <button
                      onClick={() =>
                        setSelectedImage((prev) =>
                          prev === 0 ? proxiedImages.length - 1 : prev - 1,
                        )
                      }
                      className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white p-2 rounded-full shadow-md text-gray-800 transition-all opacity-0 group-hover:opacity-100 focus:opacity-100 z-10"
                    ...
                    {/* Przycisk W Prawo (Następne) */}
                    <button
                      onClick={() =>
                        setSelectedImage((prev) =>
                          prev === proxiedImages.length - 1 ? 0 : prev + 1,
                        )
                      }
                      className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white p-2 rounded-full shadow-md text-gray-800 transition-all opacity-0 group-hover:opacity-100 focus:opacity-100 z-10"
            ...
            <div className="bg-white dark:bg-secondary-800 rounded-xl sm:rounded-2xl p-4 sm:p-6 md:sticky md:top-24 overflow-hidden">
              ...
              {product?.sku && (
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-2 sm:mb-3">
                  ...
                  <span className="font-mono">
                    {product.sku.replace(
                      /^(hp-|leker-|btp-|dofirmy-|outlet-|ikonka-)/i,
                      "",
                    )}
                  </span>
                </p>
              ...
              {isOutletProduct && (
                <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-lg p-3 mb-3 sm:mb-4">
                  <div className="flex items-start gap-2">
                    <svg
                      className="w-4 h-4 sm:w-5 sm:h-5 text-amber-500 mt-0.5 flex-shrink-0"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                    ...
                    <div>
                      <p className="text-xs sm:text-sm font-semibold text-amber-800 dark:text-amber-300 mb-0.5">
                        Produkt outletowy
                      </p>
                      <p className="text-[11px] sm:text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
                        Produkt może posiadać uszkodzone opakowanie. Objęty
                        pełną gwarancją, identyczną jak przy zakupie nowego
                        produktu.
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setQuantity(
                            clampQuantity(Number(e.target.value || 1)),
                          )
                        }
              ...
              {(() => {
                ...
                if (
                  ...
                  tags.some(
                    (t) =>
                      t.toLowerCase() === "rzeszów" ||
                      t.toLowerCase() === "outlet",
                  )
                ) {
                  warehouseCity = "Rzeszowie";
                } else if (
                  tags.some((t) =>
                    t.toLowerCase().includes("hurtownia przemysłowa"),
                  ) ||
                  blId.startsWith("hp-")
                ) {
                  ...
                  tags.some((t) => t.toLowerCase() === "leker") ||
                  blId.startsWith("leker-")
                ) {
                  warehouseCity = "Chynowie";
                } else if (
                  tags.some((t) => t.toLowerCase() === "btp") ||
                  blId.startsWith("btp-")
                ) {
                  warehouseCity = "Chotowie";
                } else if (
                  tags.some((t) => t.toLowerCase() === "dofirmy") ||
                  blId.startsWith("dofirmy-")
                ) {
                  warehouseCity = "Koszalinie";
                } else if (
                  tags.some((t) =>
                    t.toLowerCase().includes("hurtownia kuchenna"),
                  ) ||
                  blId.startsWith("hk-")
                ) {
              ...
              {(() => {
                ...
                // Check if gabaryt
                const gabarytTag = tags.find((t) =>
                  /^((\d+(?:\.\d{2})?)\s*)?gabaryt$/i.test(t),
                );
                const isGabaryt = !!gabarytTag;
                const gabarytMatch = gabarytTag?.match(
                  /^(\d+(?:\.\d{2})?)\s*gabaryt$/i,
                );
                const gabarytPrice = gabarytMatch
                  ? parseFloat(gabarytMatch[1])
                  : 49.99;

                // Check if courier only ("Tylko kurier")
                const isCourierOnly = tags.some((t) =>
                  /^tylko\s*kurier$/i.test(t),
                );
                  ...
                  <div className="border-t dark:border-secondary-700 pt-3 sm:pt-4 space-y-2 sm:space-y-3">
                    ...
                    <div className="space-y-1.5 sm:space-y-2">
                      {methods.map((method, idx) => {
                        const isFree =
                          method.freeEligible && qualifiesForFreeShipping;
                        return (
                          <div
                            key={idx}
                            className="flex items-center justify-between py-1.5 sm:py-2 px-2 sm:px-3 rounded-lg bg-gray-50 dark:bg-secondary-700/50"
                          >
                    ...
                    {!qualifiesForFreeShipping && !isGabaryt && (
                      <p className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">
                        ...
                        Darmowa wysyłka od {freeShippingThreshold} zł (brakuje{" "}
                      ...
                      <div className="space-y-3">
                        ...
                        {manufacturer.euRepEmail && (
                          <div>
                            ...
                            <a
                              href={`mailto:${manufacturer.euRepEmail}`}
                              className="text-sm text-primary-600 dark:text-primary-400 hover:underline"
                            >
                              {manufacturer.euRepEmail}
                            </a>
                        ...
                        <div className="mb-6">
                          ...
                          {canReviewInfo && !canReviewInfo.hasPurchased && (
                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                              Twoja opinia będzie oznaczona jako
                              niezweryfikowana. Opinie zweryfikowane mogą
                              dodawać tylko klienci, którzy zakupili ten
                              produkt.
                            </p>
                          )}
                        </div>
                    ...
                    {!isAuthenticated && (
                      <div className="mb-6 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <a
                            href="/login"
                            className="text-sm font-medium text-orange-500 hover:text-orange-600 underline"
                          >
                            Zaloguj się
                          </a>
                        </div>
                      ...
                      <div className="flex justify-between items-center mb-6">
                        <div className="flex gap-2">
                          <select
                            ...
                            onChange={(e) =>
                              setReviewsSortBy(
                                e.target.value as typeof reviewsSortBy,
                              )
                            }
                    ...
                    {reviews.length > 0 ? (
                      <div className="space-y-6">
                        {reviews.map((review) => (
                          <div
                            key={review.id}
                            className="border-b border-gray-200 dark:border-secondary-700 pb-6 last:border-0"
                          >
                            ...
                            <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                              ...
                              <button
                                onClick={() =>
                                  handleMarkHelpful(review.id, true)
                                }
                              ...
                              <button
                                onClick={() =>
                                  handleMarkHelpful(review.id, false)
                                }
                                disabled={!!votedReviews[review.id]}
                                className={`flex items-center gap-1 transition-colors ${
                      ...
                      <div className="text-center py-12">
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mb-6 max-w-sm mx-auto">
                          Ten produkt nie ma jeszcze żadnych opinii. Bądź
                          pierwszą osobą, która podzieli się swoją opinią!
                        </p>
                        {!isAuthenticated && (
                          <a
                            href="/login"
                            className="inline-block text-sm font-medium text-orange-500 hover:text-orange-600 underline"
                          >
