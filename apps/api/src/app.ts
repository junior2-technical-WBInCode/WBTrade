...
app.listen(PORT, async () => {
  ...
  // AUDYT V-05: zadania tła wydzielone do ./workers/background-jobs.
  // Gdy działa osobny serwis workera (node dist/worker.js na Render),
  // ustaw DISABLE_IN_PROCESS_WORKERS=true na API, aby nie dublować zadań
  // i nie obciążać heapu procesu HTTP ciężką synchronizacją.
  if (process.env.DISABLE_IN_PROCESS_WORKERS === 'true') {
    console.log('ℹ️  In-process background jobs disabled (DISABLE_IN_PROCESS_WORKERS=true) — separate worker service handles them.');
  } else {
