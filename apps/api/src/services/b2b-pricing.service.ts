...
export async function calculateB2bPriceForProduct(
  ...
  if (whKey) {
    ...
    if (rulesConfig && Array.isArray(rulesConfig.rules) && rulesConfig.rules.length > 0) {
      // 1. Get retail config to reverse the price to wholesale
      const retailConfig = await getRetailConfig(whKey);
      const wholesalePrice = reverseRetailPriceToWholesale(price, retailConfig.rules, retailConfig.divider);
  ...
  const b2bPrice = basePrice * b2bInfo.multiplier;
  return roundPriceTo99(b2bPrice);
}
